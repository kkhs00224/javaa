...
타입 어노테이션은 Java SE 8 이상부터 사용 가능하며, 우리가 사용하는 타입의 어디에든 어노테이션을 사용할 수 있다. 타입이 사용되는 몇 가지 예로, 클래스 인스턴스 생성 표현식인 ```new```, 캐스트, ```implements```절, ```throws```절 등이 있다.   
  이런 타입에 사용되는 어노테이션을 타입 어노테이션(type annotation)이라고 부른다. 타입 어노테이션은 자바 프로그램의 분석을 개선하고 더 강력한 타입 검사를 보장하기 위해 만들어졌다. Java SE 8에서는 타입 검사를 위한 프레임워크를 제공하지 않으며, 자바 컴파일러와 함께 사용되 플러그인 모듈로 구현되는 타입 검사 프레임워크를 다운로드하거나 작성할 수 있다.   

  예를 들어, 프로그램에서 특정 변수에 null을 절대로 허용하지 않음을 보장하기 원하고, ```NullPointerException```이 발생하는 것을 피하려고 한다. 이를 검사하기 위한 커스텀 플러그인을 작성할 수 있다.   
  많은 경우에는 직접 타입 검사 모듈을 작성할 수 없기 때문에 이미 누군가가 만들어 놓은 서드 파티 모듈을 사용하기도 한다. 그 중 하나가 워싱턴 대학(University of Washington)에서 만들어진 [Checker Framework](https://checkerframework.org/)를 활용할 수도 있다. 이 프레임워크에는 정규 표현식 모듈과 뮤텍스 잠금 모듈, 앞서 말한 NullPointerException을 피하기 위한 ```NonNull``` 모듈도 포함되어 있다.   
  앞선 2 챕터에서 어노테이션의 사용 위치를 알아보기 위해 사용한 예시에서 나왔던 ```@NonNull``` 타입 어노테이션이 실제 작동하지 않았던 것도 이를 위한 모듈을 직접 작성을 하거나, Checker Framework를 사용하지 않았기 때문이다.

 ## 6. 반복 어노테이션(Reapeating Annotations)
   선언이나 타입 사용에서 같은 어노테이션을 적용해야 하는 상황이 생길 수 있다. Java SE 8부터는 반복 어노테이션을 활용할 수 있다.  
   예를 들어, UNIX 크론 서비스와 비슷하게 특정 스케줄이나 주어진 시간에 따라 메서드를 실행하는 것을 가능하게 하는 타이머 서비스를 사용한 코드를 작성한다고 가정한다. ```doPeriodocCleanup()```이라는 메서드의 실행을 매월 마지막 날과 매주 금요일 오후 11시로 타이머를 설정하길 원한다면, ```@Schedule``` 어노테이션을 생성하고 해당 메서드에 두 번 적용하면 된다. 아래의 코드는 메서드에 커스텀 어노테이션을 적용시켜본 예이다.
```java
@Schedule(dayOfMonth="last")
@Schedule(dayOfWeek="Fri", hour="23")
public void doPeriodicCleanup() { ... }
```
표준 어노테이션을 사용할 수 있는 곳이라면 어디든 반복 어노테이션을 사용할 수 있다.   
  호환성 문제를 위해서, 반복 어노테이션은 자바 컴파일러에 의해 자동으로 생성된 컨테이너 어노테이션(container annotation)에 저장된다. 컴파일러가 이를 수행하기 위해서는 코드에서 두 개의 선언이 필요하다.

  ### Step 1: 반복 가능한 어노테이션 타입 선언(Declare a Repeatable Annotation Type)
  어노테이션 타입은 반드시 ```@Repeatable``` meta-annotation으로 표시되어야 한다.
  ```java
...
public @interface Schedule {
 ...
 int hour() default 12;
  ...
  ```
```@Repeatable``` 메타 어노테이션의 괄호 안에 들어간 값(value)은 자바 컴파일러에서 생성하는 컨테이너 어노테이션 타입이다. 위 예시에서는 컨테이닝 어노테이션 타입을 ```@Schedules```로 지정했고, 반복될 ```@Schedule``` 어노테이션은 이 ```@Schedule``` 어노테이션에 저장된다.   
  만약 반복 가능하다고(repeatable) 선언하지 않고 같은 어노테이션을 적용하면 컴파일 시간 오류(compile-time error)가 발생한다.

 ### Step 2: 컨테이닝 어노테이션 타입 정의(Declare the Containing Annotation Type)
 컨테이닝 어노테이션 타입은 반드시 배열(array) 타입의 값(value) 요소를 가져야 하고, 배열(array) 타입의 구성 요소 타입은 반복 어노테이션 타입이어야 한다.
 ```java
...
### 어노테이션 검색(Retrieving Annotations)
어노테이션 검색에 용할 수 있는 Reflection API 안에는 여러 개의 메소드가 있다. [AnnotatedElement.getAnnotation(Class<T>)](https://docs.oracle.com/en/java/javase/20/docs/api/java.base/java/lang/reflect/AnnotatedElement.html#getAnnotation(java.lang.Class))와 같은 단일 어노테이션을 반환하는 메소드의 동작은, 만약 요청된 어노테이션이 하나만 존재할 때 단일 어노테이션을 반환하는 것으로 변하지 않는다. 요청된 어노테이션 타입이 하나 이상 존재할 때에는 먼저 컨테이너 어노테이션을 얻어올 수 있다. 이런 방식으로 레거시 코드가 계속 동작한다.   
  컨테이너 어노테이션을 통해 한 번에 여러 어노테이션을 반환하는 [AnnotatedElement.getAnnotationsByType(Class<T>)](https://docs.oracle.com/en/java/javase/20/docs/api/java.base/java/lang/reflect/AnnotatedElement.html#getAnnotationsByType(java.lang.Class))와 같은 메소드도 도입되어 있다.

### 설계 고려사항 (Design Considerations)
어노테이션 타입을 설계할 때, 그 타입의 어노테이션이 가지는 카디널리티(cardinality)를 고려해야만 한다. 어노테이션을 0번, 1번 또는 ```@Repeatable```로 표시된 여러 번 사용할 수 있다.   
[@Target](https://docs.oracle.com/en/java/javase/20/docs/api/java.base/java/lang/annotation/Target.html) 메타 어노테이션을 사용해 어노테이션 타입이 사용될 수 있는 위치를 제한할 수도 있다.
